...
def _secret_input(label: str, current: str | None, key: str, hint: str = "") -> str:
    ...
    ph = (
        ...
        if current
        else (hint or "Pega tu clave aquí")
    )
...
def parse_price(s) -> float | None:
    try:
        return float(
            str(s or "").replace("$", "").replace("S/", "").replace(",", "").strip()
        )
...
def _run_async(coro):
    """Runs a coroutine to completion in a fresh event loop, closing it afterwards.

    Streamlit re-executes this script top-to-bottom on every interaction, outside
    any asyncio context — each call here must get its own loop and clean up after
    itself (the async checkpointer connections are opened/closed per call too,
    see shared/checkpointing.py, precisely so they never outlive a closed loop)."""
...
with st.sidebar:
    ...
    _dot(
        telegram_token and groq_key and telegram_allowed,
        "Agente bot activo",
        "Bot sin configurar",
    )
    ...
    query = st.text_input(
        "Modelo de auto", value="", placeholder="ej. Toyota Corolla 2015"
    )
    ...
    preset_label = st.radio(
        "Segmento", list(_PRESETS.keys()), index=0, label_visibility="hidden"
    )
...
with tab_search:
    if not search_btn:
        ...
        scraper = FacebookScraper(
            city=city, min_price=min_price, max_price=max_price, query=query
        )
        autos_scraped = _run_async(scraper.scrape_cars(max_items))
        pb.progress(
            30, text=f"Encontrados {len(autos_scraped)} autos. Analizando con IA..."
        )

        if not autos_scraped:
            pb.empty()
            st.warning(
                "No se encontraron autos. Facebook puede estar bloqueando el acceso "
            ...
            for idx, auto in enumerate(autos_scraped, 1):
                pb.progress(
                    30 + int(idx / len(autos_scraped) * 65),
                ...
                try:
                    state = _run_async(orchestrator.run_acquisition(car_data=auto))
                ...
                is_apto = (
                    getattr(state, "status", None) in ("acquired", "published")
                ...
                if is_apto:
                    label = (
                        f"✅ **¡Buena oferta!** — Ganancia estimada: **${gan:,.0f}** ({pct_gan:.0f}%)"
                        if gan
                ...
                with col_body:
                    ...
                    if is_apto and gan:
                        ...
                        if margen:
                            pill += f" ({margen:.0f}%)"
                        st.markdown(
                            f'<span class="profit-pill">{pill}</span>',
                            unsafe_allow_html=True,
                        )

                    # Precio publicado con moneda detectada
                    moneda = state.car_data.get("moneda_detectada") or auto.get(
                        "currency", ""
                    )
                    ...
                    if is_apto:
                        m3.metric(
                            "Precio máximo de compra", f"${pv:,.0f}" if pv else "—"
                        )
                    else:
                        limit = pm * 0.82 if pm else None
                        m3.metric(
                            "Precio que sería rentable",
                            f"${limit:,.0f}" if limit else "—",
                        )

                    # Meta info: condition, km, transmission, fuel, currency note
                    meta_parts = []
                    if auto.get("condition"):
                        meta_parts.append(f"🔧 {auto['condition']}")
                    if auto.get("kilometraje"):
                        meta_parts.append(f"🛣️ {auto['kilometraje']:,} km")
                    if auto.get("transmision"):
                        meta_parts.append(f"⚙️ {auto['transmision']}")
                    if auto.get("combustible"):
                        meta_parts.append(f"⛽ {auto['combustible']}")
                    if moneda:
                        meta_parts.append(f"💱 {moneda}")
                    if meta_parts:
                    ...
                    if is_apto:
                        ...
                        with st.expander("📣 Texto listo para publicar"):
                            ...
                            if pub.get("descripcion"):
                                ...
                                st.markdown(
                                    f'<div class="pub-box">{pub["descripcion"]}</div>',
                                    unsafe_allow_html=True,
                                )
                            if pub.get("precio_venta"):
                                st.markdown(
                                    f"**Precio sugerido de venta:** ${pub['precio_venta']:,.0f}"
                                )
                        ...
                        if telegram_token and telegram_chat:
                            ...
                            ).send_deal_alert(auto, state.car_data)
            ...
            omitidos_txt = (
                f" · {ya_vistos} ya analizados antes (omitidos)" if ya_vistos else ""
            )
            if aptos_count > 0:
                banner.success(
...
with tab_manual:
    ...
    st.caption(
        "¿Encontraste un auto en un grupo de WhatsApp, de un amigo, o en otra plataforma? "
        "Agrégalo aquí y la IA calculará si es buena oportunidad."
    )

    with st.form("form_manual"):
        fc1, fc2 = st.columns(2)
        m_title = fc1.text_input(
            "Título / Descripción corta *", placeholder="ej. Toyota Corolla 2018"
        )
        m_price = fc2.text_input(
            "Precio publicado *", placeholder="ej. $8500 o S/12000"
        )
        fc3, fc4, fc5 = st.columns(3)
        m_year = fc3.number_input("Año", min_value=1990, max_value=2026, value=2015)
        m_city = fc4.text_input("Ciudad", placeholder="lima")
        m_cond = fc5.selectbox(
            "Condición",
            [
                "",
                "Usado - Buen estado",
        ...
        m_url = st.text_input(
            "URL de la publicación (opcional)", placeholder="https://..."
        )
        m_img = st.text_input("URL de la foto (opcional)", placeholder="https://...")
        m_desc = st.text_area(
            "Descripción del vendedor (opcional)",
    ...
    if submitted:
        if not m_title or not m_price:
            ...
            with st.spinner("Analizando con IA..."):
                ...
                try:
                    state2 = _run_async(orch2.run_acquisition(car_data=car_data))
                except Exception as e:
            ...
            if state2:
                is_apto2 = (
                    getattr(state2, "status", None) in ("acquired", "published")
                ...
                if is_apto2:
                    ...
                    obs2 = (
                        state2.inspection_data.get("observaciones")
                        or state2.inspection_data.get("resultado_inspeccion")
                        or ""
                    )
                    if obs2:
                    ...
                    razon2 = (
                        state2.inspection_data.get("resultado_inspeccion")
...
with tab_db:
    if not airtable.is_configured():
        ...
        if not cars:
            ...
            gan_pot = sum(
                max(
                    (c.get("Precio Mercado") or 0) - (c.get("Precio Publicado") or 0), 0
                )
                for c in cars
            )
            ...
            s1.markdown(
                f'<div class="stat-box"><div class="stat-num">{total}</div>'
                f'<div class="stat-label">Oportunidades guardadas</div></div>',
                unsafe_allow_html=True,
            )
            s2.markdown(
                f'<div class="stat-box"><div class="stat-num">${gan_pot:,.0f}</div>'
                f'<div class="stat-label">Ganancia potencial total</div></div>',
                unsafe_allow_html=True,
            )
            s3.markdown(
                f'<div class="stat-box"><div class="stat-num">${gan_real:,.0f}</div>'
                f'<div class="stat-label">Ganancia real obtenida</div></div>',
                unsafe_allow_html=True,
            )
            s4.markdown(
                f'<div class="stat-box"><div class="stat-num">{vendidos}</div>'
                f'<div class="stat-label">Autos vendidos</div></div>',
                unsafe_allow_html=True,
            )
            ...
            df_export = pd.DataFrame(
                [{k: v for k, v in c.items() if not k.startswith("_")} for c in cars]
            )
            csv_bytes = df_export.to_csv(index=False).encode("utf-8")
            tb2.download_button(
                "📥 Exportar CSV",
                csv_bytes,
                "anymotor_oportunidades.csv",
                "text/csv",
                key="csv_dl",
            )
            ...
            # Comparar
            selected_cars = [
                c for c in cars if c.get("_id") in st.session_state.compare_ids
            ]
            if len(selected_cars) >= 2:
                with st.expander(
                    f"⚖️ Comparando {len(selected_cars)} autos", expanded=True
                ):
                    ...
                    for ci, (col, car) in enumerate(zip(cols, selected_cars)):
                        ...
                        col.metric(
                            "Ganancia estimada",
                            f"${gan_c:,.0f} ({pct_c:.0f}%)" if gan_c else "—",
                        )
            ...
            for car in cars:
                ...
                with col_body:
                    ...
                    if gan_c > 0:
                        st.markdown(
                            f'<span class="profit-pill">💰 +${gan_c:,.0f} ({pct_c:.0f}%)</span>',
                            unsafe_allow_html=True,
                        )
                    ...
                    m3.metric(
                        "Precio máx. de compra", f"${venta:,.0f}" if venta else "—"
                    )

                    meta = st.columns(3)
                    if car.get("Estado"):
                        meta[0].caption(f"🔧 {car['Estado']}")
                    if car.get("Ciudad"):
                        meta[1].caption(f"📍 {car['Ciudad']}")
                    if car.get("Fecha Análisis"):
                        meta[2].caption(f"📅 {car['Fecha Análisis']}")

                    db_link_cols = st.columns([1, 1, 4])
                    if car.get("URL"):
                        db_link_cols[0].markdown(f"[🔗 Ver publicación]({car['URL']})")
                    if car.get("Teléfono"):
                        wa_url_db = whatsapp_link(
                            car["Teléfono"], title, venta or merc * 0.82 or None
                        )
                    ...
                    with st.expander("✏️ Actualizar estado y finanzas"):
                        ...
                        new_status = pc1.selectbox(
                            "Estado del deal",
                            PIPELINE_OPTIONS,
                            index=PIPELINE_OPTIONS.index(status)
                            if status in PIPELINE_OPTIONS
                            else 0,
                            key=f"pipe_{rec_id}",
                        )
                        ...
                        if new_status in ("Comprado", "Vendido"):
                            ...
                            compra_real = fc1r.number_input(
                                "Precio que pagué (real)",
                                value=float(car.get("Precio Compra Real") or 0),
                                step=100.0,
                                key=f"compra_{rec_id}",
                            )
                            venta_real = fc2r.number_input(
                                "Precio que vendí (real)",
                                value=float(car.get("Precio Venta Real") or 0),
                                step=100.0,
                                key=f"venta_{rec_id}",
                            )
                            if compra_real > 0:
                        ...
                        if pc2.button("💾 Guardar", key=f"save_{rec_id}"):
                            ...
                            if result:
                                ...
                                st.error(
                                    "No se pudo guardar. ¿Corriste el setup de campos avanzados?"
                                )
...
with tab_dash:
    ...
    if not airtable.is_configured():
        ...
        if not dash_cars:
            st.info(
                "Aún no hay datos. Empieza buscando ofertas para ver estadísticas aquí."
            )
        else:
            ...
            gan_pot_d = sum(
                max(
                    (r.get("Precio Mercado") or 0) - (r.get("Precio Publicado") or 0), 0
                )
                for r in dash_cars
            )
            ...
            with ch2:
                ...
                top8 = df.nlargest(8, "Ganancia Potencial")[
                    ["Título", "Ganancia Potencial"]
                ].copy()
            ...
            if (df["Ganancia Real"] > 0).any():
                ...
                fig4 = px.bar(
                    ...
                    barmode="group",
                    color_discrete_map={
                        "Ganancia Potencial": "#93c5fd",
                        "Ganancia Real": "#10b981",
                    },
                )
...
with tab_cfg:
    ...
    with st.expander("🗄️ Base de datos Airtable", expanded=not airtable.is_configured()):
        ...
        new_base = st.text_input(
            "Base ID (ej. appXXXXXXXXXXXXXX)",
            value=airtable.base_id or "",
            key="cfg_base",
        )
        new_tbl = st.text_input(
            "ID de tabla (ej. tblXXXXXXXXXXXXXX)",
            value=airtable.table_name or "",
            key="cfg_tbl",
        )
    ...
    with st.expander("🔧 Campos avanzados en Airtable"):
        ...
        if st.button(
            "⚙️ Configurar campos avanzados", type="primary", key="setup_fields"
        ):
            if not airtable.is_configured():
    ...
    with st.expander(
        "🤖 Agente Telegram (Bot inteligente)", expanded=not bool(telegram_allowed)
    ):
        st.markdown(
    ...
    with st.expander("💬 Notificaciones Telegram"):
        st.markdown(
            "Recibe un mensaje en Telegram cada vez que se detecte una buena oferta."
        )
        _cfg_status(telegram_token)
        new_tg_token = _secret_input("Bot Token", telegram_token, "cfg_tg_t")
        new_tg_chat = st.text_input(
            "Chat ID", value=telegram_chat or "", key="cfg_tg_c"
        )
    ...
    with st.expander("📱 Notificaciones WhatsApp"):
        ...
        **Cómo activarlo (una sola vez):**
        ...
        new_wa_phone = st.text_input(
            "Tu número de WhatsApp (con código de país, ej. +51999888777)",
            value=whatsapp.phone or "",
            key="cfg_wa_phone",
        )
        new_wa_key = _secret_input(
            "API Key de CallMeBot", whatsapp.api_key, "cfg_wa_key"
        )
        ...
        if col_test_wa.button("🧪 Enviar mensaje de prueba", key="test_wa"):
            ...
            if test_wa.is_configured():
                ok = test_wa.send(
                    "🚗 Anymotor: ¡Notificaciones de WhatsApp activadas correctamente!"
                )
                if ok:
